...
TOP_N = 2000           # топ 2000 крупнейших городов
BATCH_SIZE = 100       # сохранять каждые 100 городов
MAX_WORKERS = 15       # потоки для параллельной загрузки
...
def main():
    download_geonames()
    all_cities = load_top_cities(limit=TOP_N)
    print(f"🌍 Всего городов для обработки: {len(all_cities)}")
