...
def get_worldbank_country_data(country_code):
    ...
    try:
        ...
        print(f"⚠️ Ошибка WorldBank для {country_code}: {e}")
    return data
...
def get_who_country_data(country_code):
    ...
    for field, code in endpoints.items():
        ...
        try:
            ...
            print(f"⚠️ Ошибка WHO для {country_code}: {e}")
    return data
...
def get_openaq(city_name):
    ...
    try:
        ...
        print(f"⚠️ Ошибка OpenAQ для {city_name}: {e}")
        return {}
...
def get_climate(lat, lon):
    try:
        ...
        print(f"⚠️ Ошибка Climate API для ({lat},{lon}): {e}")
    return {"avg_temperature": None, "avg_precipitation": None}

# ---------------------------
# Трафик (TomTom Traffic Index)
# ---------------------------
...
def process_city(city, wb_data, who_data, crime_data):
    ...
    record.update(get_transport_index(city["name"]))
    return record
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for i, future in enumerate(as_completed(future_to_city), 1):
            ...
            if city_record:
                ...
                print(f"🔹 Обработан город: {city_record['name']} ({i}/{len(cities)})")
            if i % BATCH_SIZE == 0:
