...
# Настройки
# ---------------------------
OUTPUT_FILE = os.path.join("public", "data", "cities_final_with_region.json")
BACKUP_FILE = os.path.join("public", "data", "cities_final_with_region_backup.json")
...
class DataCollector:
    ...
    # Numbeo
    # ---------------------------
    async def fetch_numbeo(self, city_name: str, country_name: str) -> Dict[str, float]:
        base_url = "https://www.numbeo.com/cost-of-living/in/"
        city_slug = city_name.replace(" ", "-")
        country_slug = country_name.replace(" ", "-")
        url = f"{base_url}{city_slug}?country={country_slug}"
        text = await self.fetch_text(url)
        out = {}
        if text:
            soup = BeautifulSoup(text, "html.parser")
            salary_tag = soup.find("div", string=lambda x: x and "Median Salary" in x)
            if salary_tag:
                val_tag = salary_tag.find_next("span")
                if val_tag:
                    val = val_tag.text.replace(",", "").replace("$", "").strip()
                    try:
                        out["median_income"] = float(val)
                    except:
                        pass
            index_tag = soup.find("div", string=lambda x: x and "Cost of Living Index" in x)
            if index_tag:
                val_tag = index_tag.find_next("span")
                if val_tag:
                    val = val_tag.text.replace(",", "").strip()
                    try:
                        out["cost_of_living_index"] = float(val)
                    except:
                        pass
        return out
    ...
    # OpenStreetMap
    # ---------------------------
    async def fetch_osm_infrastructure(self, lat: float, lon: float, radius_km=10) -> Dict[str, float]:
        ...
        out = {}
        try:
            async with self.session.post(overpass_url, data=query) as r:
                if r.status != 200:
                    return {}
                js = await r.json()
                counts = js.get("elements", [])
                out["transport_points_count"] = sum([e.get("count", 0) for e in counts])
                out["road_density_km2"] = len([e for e in counts if e.get("type")=="way"])
        except:
            pass
        return out
    ...
    async def fetch_city_data(self, city: Dict[str, Any]) -> Dict[str, Any]:
        ...
        country_name = self.iso2_to_iso3.get(iso2, {}).get("name")
        ...
        rec = {
            ...
            "region_name": ISO2_TO_CONTINENT.get(iso2, "Unknown")
        }

        tasks = [
            self.fetch_meteo(city["lat"], city["lon"]),
            self.fetch_openaq(city["name"]),
            self.fetch_osm_infrastructure(city["lat"], city["lon"]),
        ...
        if country_name:
            tasks.append(self.fetch_numbeo(city["name"], country_name))
        ...
        if country_name:
            rec.update(results[-1])

        if "transport_points_count" in rec and "population" in rec and rec["population"]>0:
            rec["transport_points_per_100k"] = round(rec["transport_points_count"] / rec["population"] * 100000, 2)
...
async def main_async():
    ...
    print(f"✅ Готово. Записано {len(results)} городов → {OUTPUT_FILE}")
