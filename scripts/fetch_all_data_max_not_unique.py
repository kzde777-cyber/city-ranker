...
# ---------------------------
# ---------------------------
# ...код до main_async() без изменений...
# ---------------------------
...
async def main_async():
    ...
    # ---------------------------
    # Нормализация income_index по медиане
    # ---------------------------
    income_values = [c.get("gni_per_capita") for c in results if c.get("gni_per_capita") is not None]
    if income_values:
        median_income = sorted(income_values)[len(income_values)//2]
        for city in results:
            gni = city.get("gni_per_capita")
            if gni is not None and median_income > 0:
                city["income_index"] = gni / median_income
                gdp_usd = city.get("gdp_per_capita")
                if gdp_usd and gdp_usd > 0:
                    city["cost_index"] = gni / gdp_usd

    # ---------------------------
    # Фильтрация по ключевым данным
    # ---------------------------
    required_keys = [
